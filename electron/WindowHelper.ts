...
// Try common ports that Vite might use (5180, 5181, 5182, etc.)
const tryLoadUrl = async (window: BrowserWindow, ports: number[]): Promise<void> => {
  ...
  // If all ports fail, try the first one anyway
  console.warn("Could not connect to Vite dev server, trying default port")
  await window.loadURL(`http://localhost:${ports[0]}`)
}

const startUrl = isDev
  ? "http://localhost:5180" // Will try multiple ports
...
export class WindowHelper {
  ...
  public createWindow(): void {
    ...
    // In dev mode, try multiple ports (Vite may use 5180, 5181, 5182, etc.)
    if (isDev && this.mainWindow) {
      tryLoadUrl(this.mainWindow, [5180, 5181, 5182, 5183, 5184]).catch((err) => {
        console.error("Failed to load from any port:", err)
      })
    } else {
